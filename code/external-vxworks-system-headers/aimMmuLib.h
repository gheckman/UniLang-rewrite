...
--------------------
01v,28feb12,h_k  made TLB sync config flexible to be able to share one binary
                 with different CPU variants. (WIND00320752)
01u,01feb11,slk  add mmuTlbPoolCreateHook hook for region and page table pool
                 creation
01t,21jul10,wiq  Remove useless typedef. (WIND00054776)
01t,09jun10,d_c  Add mmuPostStateSetHook to MMU_ARCH_LIB_FUNCS.
01s,29apr10,pad  Moved extern C statement after include statements.
01r,14jan10,cwl  Remove the function aimMmuShowInit(). (WIND00184839)
01q,29nov06,sru  Added complete prototypes and header comments for the
                 function pointers that make up the AD-MMU interface.
01p,29jan07,h_k  added an extra argument to MODIFYLOCALPTEFUNC.
...
typedef struct
     ...
     * the the memory management hardware for the CPU.
     */
	...
	UINT newPageSize,	/* new page size */
	...
	UINT numBytes		/* number of bytes of data to copy */
	);

    /*
     * mmuAttrTranslate - Optional AD-MMU function.  If provided by the
     * AD-MMU, this function is used to translate state information
       between architecture-independent and architecture-dependent forms.
     */

    STATUS (*mmuAttrTranslate)
	(
	UINT fromState,			/* starting state */
	UINT * pToState,		/* ending state */
	UINT fromStateMask,		/* starting state mask */
	UINT * pToStateMask,		/* ending state mask */
	BOOL independentoToDependent	/* conversion direction */
	);
	...
	void * pPte,			/* pointer to MMU-dependent PTE */
	VIRT_ADDR virtAddr,		/* virtual address for PTE */
	UINT size,			/* of address range in bytes */
    ...
    STATUS (*mmuPostStateSetHook)
        ...
        UINT numPages,            /* Number of pages in memory block */
        UINT pageSize,            /* Page size in memory block */
