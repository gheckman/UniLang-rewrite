/* shSciSio.h - header file for Renesas SH on-chip SCI serial driver */

/*
 * Copyright (c) 1996-2006, 2010 Wind River Systems, Inc.
 *
...
01s,18nov03,h_k  added SCI_SCR_CKE and SCI_SMR_CKS definitions.
01r,10nov03,h_k  corrected the comment for the BRR formula.
01q,06nov03,h_k  added SH7705 support. (SPR #91873)
...
#define	SCI_SMR_CKS		0x03	/* clock select bits */
...
#define SMR_VALUE(x)	(((SCI_FREQ/(BRR_FORM_VAL*32*(x))) >= 64) ? 3 : \
			 ((SCI_FREQ/(BRR_FORM_VAL*32*(x))) >= 16) ? 2 : \
			 ((SCI_FREQ/(BRR_FORM_VAL*32*(x))) >=  4) ? 1 : 0)

#define CLK_DIVIDE(y)	(((SCI_FREQ/(BRR_FORM_VAL*32*(y))) >= 64) ? 64 : \
		 	 ((SCI_FREQ/(BRR_FORM_VAL*32*(y))) >= 16) ? 16 : \
		 	 ((SCI_FREQ/(BRR_FORM_VAL*32*(y))) >=  4) ?  4 : 1)

#define BRR_FORMULA(s)	(((20*SCI_FREQ)/ \
...
#define	SCI_SCR_CKE		0x03	/* clock enable bits */
