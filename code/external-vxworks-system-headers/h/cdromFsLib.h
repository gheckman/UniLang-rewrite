...
02m,20dec05,pcm  added parameter <commonbBufferSize> to cdromFsInit()
02l,11aug05,aeg  moved cdromFsDevDelete() prototype from cdromFsLibP.h
02k,30apr05,aeg  deprecated cdromFsVersionNumGet() and cdromFsVersionDisplay();
		 removed inclusion of private/cdromFsLibP.h;
		 changed cdromFsDevCreate() to accept device_t not BLK_DEV *;
		 removed prototype for non-existent function cdromFsDevReset().
02j,08apr05,jge  increased CDROM_MAX_FILE_NAME_LEN after seeing CDs that
		 break the Joliet 128-byte filename limit.
...
#define CDROM_MAX_FILE_NAME_LEN	256
...
extern STATUS cdromFsInit (UINT32 commonBufferSize);

/* user-callable functions prototypes */

extern CDROM_VOL_DESC_ID cdromFsDevCreate (char * devName, device_t device);
extern STATUS 		 cdromFsDevDelete (CDROM_VOL_DESC_ID volDescId);

extern void cdromFsVolConfigShow          (void * arg);

extern uint32_t cdromFsVersionNumGet      (void) _WRS_DEPRECATED ("");
extern void cdromFsVersionDisplay         (int level) _WRS_DEPRECATED ("");
