...
--------------------
01m,07jul10,pcs  change the data type of timeout from an int to _Vx_ticks_t
01l,06may10,cww  64-bit SMP update
01k,23mar10,gls  fixed build warnings
01j,14dec09,kk   fix user build warnings (WIND00194092)
01i,21sep09,s_s  changed INT_CPU_LOCK() and INT_CPU_UNLOCK() to
                 KERNEL_INT_CPU_LOCK() and KERNEL_INT_CPU_UNLOCK()
		 (WIND00181479)
01h,26aug09,gls  removed duplicate documentation (WIND00179315)
01g,06aug09,jpb  Fixed build warnings.
01f,28may09,kk   fix doc for SEM_INVERSION_SAFE
01f,15may09,gls  removed memory barrier for recursive mutex give
01e,12mar09,jpb  VSB consideration.
01h,29may09,cww  Changed 3rd param in workQAdd2
01g,18mar09,cww  LP64 update
01f,27feb09,cww  update workQAdd for LP64
01e,11feb09,zl   removed qLib.h inclusion
01d,11sep08,gls  moved check for consistent options to after ID validation
                 added SEM_EVENTSEND_ERR_NOTIFY to options mask (Defect #132659)
01c,27aug08,jpb  Renamed VSB header file
01b,04aug08,gls  added check for interrupt context before event send
01a,04jul08,gls  written
*/
...
_WRS_INLINE STATUS semMGive_inline
	 ...
	 * pend) on the mutex sempahore, i.e. the SEMM_PEND_TASK bit is set,
	...
	if (vxCas_inline ((atomic_t *) (void *) &semId->semOwner, 
	    ...
	    semMGiveKernWork (semId, kernWork, tCurrent);
     ...
     * requested the sempahore and is pending on the semaphore (or is
     * currently in the process of pending on the sempahore) and thus the
...
_WRS_INLINE STATUS semMTake_inline
    ...
    /*
     * The acquisition of a mutex sempahore is optimized for the common
     * case: The sempahore is currently full, i.e. there is no contention
	 ...
	 * prior to the memory write accesses asssociated with the give 
	...
	if (options & SEM_INVERSION_SAFE)
	    ...
	    vxAtomic32Inc_inline((atomic32_t *) &tCurrent->priMutexCnt);
	    }
    ...
    /* 
     * The mutex sempahore could not be acquired via vxCas(); return ERROR if
...
_WRS_INLINE STATUS semBGive_inline
     ...
     * Perform a full memory barrier operation in the event the fast
     * path is executed.  If a task is currently pending (or about to
     * pend) on the binary sempahore, i.e. the SEMB_PEND_TASK bit is set,
     * the kernel will be entered.  The process of entering/exiting the
     * kernel constitues a full memory barrier, and thus the following
     * full memory fence will be redundant in that case.
     */
     ...
     * attempted to acquire the sempahore while empty.  Note that releasing
     * a binary sempahore that is already full should execute the fast
...
_WRS_INLINE STATUS semBTake_inline
    ...
    /*
     * The acquisition of a binary sempahore is optimized for the common
     * case: The sempahore is currently full, i.e. there is no contention
     * for ownership of a resource.  For SMP, a CAS operation is performed
     * to atomically update the 'semOwner' field, and thus an OBJ_LOCK()
     * and OBJ_UNLOCK() operation is skipped.
    ...
    /* 
     * The binary sempahore could not be acquired via vxCas(); return ERROR if
     * called with a timeout of NO_WAIT.
     */
