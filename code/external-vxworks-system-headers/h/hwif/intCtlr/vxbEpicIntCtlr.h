/* vxbEpicIntCtlr.h - Embedded Programmable Interrupt Controller driver */

/*
 * Copyright (c) 2006-2010, 2012-2014 Wind River Systems, Inc.
 *
...
--------------------
05mar14,d_l  add Global Revision and critical Registers
01x,04feb13,j_z  add share message bank A,B,C support.(WIND00333514)
01y,28feb13,h_k  added T4_ERRATUM_CPU6198 for e6500.
01x,09jan13,zl   decouple EPIC ID and CPU ID
01w,06mar12,wqi  correct 4080 core 4 to 7 can't receive doorbell 
                 interrupt. (WIND00288544)
01v,11jan12,agf  fix compiler warnings
01u,29apr10,pad  Moved extern C statement after include statements.
01t,05jan09,b_m  add global timer group B support.
01s,09dec08,pmr  add external proxy support
01r,29oct08,pmr  fix OPT_EN_N value
01p,02jun08,h_k  split vxbEpicIntCtlr.c.
01o,30may08,pmr  flexible interrupt assignment
01n,24apr08,pmr  initial e500mc support
01m,31mar08,dtr  Modify IPI vector to work with Linux.
01l,06mar08,h_k  updated to use global access registers.
01k,16jul07,pmr  cleanup
01j,27jun07,dtr  Additonal register for shared message interrupt.
01i,11jun07,pmr  IPI priority, add FRR definitions
01h,12mar07,kk   reinstate PCI and SMSG support
01g,12mar07,kk   backed out PCI and SMSG support temporarily, breaks SMP
01f,26feb07,dtr  Support Message Shared Interrupts.
01e,09Nov06,tor  VxBus port
01d,22jun06,mmi  replaced int with cpuset_t for processor references
01c,05may06,mig  Add CPU1 IPI support
01b,18apr06,dtr  Add IPI support.
01a,06mar06,dtr  Modified from cds8548/sysEpic.h/01b
*/

#ifndef	__INCvxbSysEpich
...
/* Shared message interrupt registers */
#define EPIC_MSG_STATE_REG	(EPIC_CCSROFF + 0x01510)/* Message Status */
#define EPIC_MSIR_INT0_STATE_REG (EPIC_CCSROFF + 0x01600)/* interrupt status*/
#define EPIC_MSIR_INTX_OFFSET    0x10

#define EPIC_MSISR_REG          (EPIC_CCSROFF + 0x01720)/* interrupt set*/
#define EPIC_MSIIR_REG          (EPIC_CCSROFF + 0x01740)/* interrupt set*/
...
#define VXB_EPICINTCTLR_DISABLE_INTS(intMask)	intCpuLock()
#else
...
#define EPIC_RES_PER_VECTOR        32           /* Each Vector has 32 resource */
#define EPIC_MAX_SMSG_RESOURCE     768          /* BankA, B, C 3 * 8 * 32 */
#define EPIC_MAX_SMSG_VEC          24           /* BankA, B, C 3 * 8 */
#define EPIC_MAX_1BANK_RESOURCE    256          /* The resoure number in one Bank 8 * 32 */
#define EPIC_BASE                  (EPIC_CCSROFF + (UINT32)pEpicRegBase) /* EPIC controller register base */

#define EPIC_SMSG_VEC_STAT         EPIC_DRV_CTRL(smsgIrq0)
#define EPIC_SMSG_VEC_NUM          (EPIC_DRV_CTRL(intBankDesc[5]->numIrqs))
#define MIN(arg1, arg2)            ((arg1) < (arg2)) ? (arg1) : (arg2)

/* Shared message signaled interrupt index register */

#define EPIC_MSIIRA                0x1740
#define EPIC_MSIIRB                0x1940
#define EPIC_MSIIRC                0x1b40

/* Shared Message Signaled Interrupt Registers */

#define EPIC_MSIRA0                0x1600
#define EPIC_MSIRB0                0x1800
#define EPIC_MSIRC0                0x1A00

#define EPIC_MISRA_X(pin)          (EPIC_CCSROFF + EPIC_MSIRA0 + (pin)* 0x10)
#define EPIC_MISRB_X(pin)          (EPIC_CCSROFF + EPIC_MSIRB0 + (pin)* 0x10)
#define EPIC_MISRC_X(pin)          (EPIC_CCSROFF + EPIC_MSIRC0 + (pin)* 0x10)

/* macros to fields in interrupt controller pin array */

#define EPIC_INT_INDEX_GET(pEnt,inputPin, index)         \
        {                                                \
...
#define EPIC_INT_INDEX_SET(pEnt,inputPin, index)         \
        {                                                \
        struct vxbIntCtlrPin * pPin =                    \
           vxbIntCtlrPinEntryGet(pEnt,inputPin);         \
        pPin->intIndex = index;                          \
        }
#define EPIC_DESTCPU_GET(pEnt,inputPin,destCpu)          \
        {                                                \
        struct vxbIntCtlrPin * pPin =                    \
           vxbIntCtlrPinEntryGet(pEnt,inputPin);         \
        destCpu = pPin->pinCpu;                          \
...
#define EPIC_DESTCPU_SET(pEnt,inputPin,destCpu)          \
        {                                                \
        struct vxbIntCtlrPin * pPin =                    \
           vxbIntCtlrPinEntryGet(pEnt,inputPin);         \
        pPin->pinCpu = destCpu;                          \
...
#define EPIC_ISR_GET(pEnt,inputPin,funcPtr)              \
        {                                                \
        struct vxbIntCtlrPin * pPin =                    \
           vxbIntCtlrPinEntryGet(pEnt,inputPin);         \
        funcPtr = pPin->isr;                             \
...
#define EPIC_ARG_GET(pEnt,inputPin,argument)             \
        {                                                \
        struct vxbIntCtlrPin * pPin =                    \
           vxbIntCtlrPinEntryGet(pEnt,inputPin);         \
        argument = pPin->pArg;                           \
...
#define EPIC_NAME_SET(pEnt,inputPin,name)                \
        {                                                \
        struct vxbIntCtlrPin * pPin =                    \
           vxbIntCtlrPinEntryGet(pEnt,inputPin);         \
         pPin->drvName = name;                           \
...
#if defined (_WRS_HAS_DCC_ASM_SYNTAX)
__asm volatile void _VXB_EPICINTCTLR_ENABLE_INTS (int intMask)
{
% reg intMask
! "r7"				/* r7 clobbered */
	mfmsr	r7		/* move MSR to r7 */
	or	r7, r7, intMask	/* restore EE (and CE) bit */
	mtmsr	r7		/* UNLOCK INTERRUPTS */
}

__asm volatile void _VXB_EPICINTCTLR_DISABLE_INTS (int intMask)
{
% reg intMask
! "r7"				/* r7 clobbered */
	mfmsr	r7		/* move MSR to r7 */
	andc	r7, r7, intMask	/* mask EE (and CE) bit */
	mtmsr	r7		/* LOCK INTERRUPTS */
}
#elif defined (_WRS_HAS_GCC_ASM_SYNTAX)
...
#else	/* _WRS_HAS_DCC_ASM_SYNTAX */
#error The compiler does not support this type of in-line assembly
#endif	/* _WRS_HAS_DCC_ASM_SYNTAX */

#ifdef	_WRS_CONFIG_SMP
/* function pointer points to the EPIC ID getter, that can be set by BSP */

extern UINT (*_func_vxbSysEpicIdGet) (UINT);

extern void vxbSysEpicIdAssign (UINT cpuCount, BOOL epicIdInterval);
extern UINT vxbSysEpicIdGet (UINT physCpuIndex);
#endif	/* _WRS_CONFIG_SMP */

extern VOIDFUNCPTR _func_vxbSysEpicIntHandler;
extern VXIPI_EMIT_FUNC _func_vxEpicIpiCtrlInitIpiEmit;

extern void vxbSysEpicIntHandler (void);
extern STATUS vxbEpicIpiGen
    (
    VXB_DEVICE_ID pCtlr,
    INT32 ipiId,
    phys_cpuset_t cpus
...
#ifdef	PPC_e6500
extern void vxbSysEpicE6500IntHandler (void);
extern STATUS vxbEpicE6500IpiGen
    (
    VXB_DEVICE_ID pCtlr,
    INT32 ipiId,
    phys_cpuset_t cpus
...
#define EPIC_REG_READ(offset)	\
    vxbRead32 (EPIC_DRV_CTRL(regAccessHandle), \
	       (UINT32 *)((UINT32)pEpicRegBase + (offset & REG_ADRS_MASK)))

#define EPIC_REG_WRITE(offset, value)	\
    vxbWrite32 (EPIC_DRV_CTRL(regAccessHandle), \
		(UINT32 *)((UINT32)pEpicRegBase + (offset & REG_ADRS_MASK)), \
		value)

#define EPIC_REG_NO_OFFSET_READ(regAdrs)  \
    vxbRead32 (EPIC_DRV_CTRL(regAccessHandle), (UINT32 *)(regAdrs))

#define EPIC_REG_NO_OFFSET_WRITE(regAdrs, value)  \
    vxbWrite32 (EPIC_DRV_CTRL(regAccessHandle), (UINT32 *)(regAdrs), value)
#else	/* VXB_EPICINTCTLR_ACCESS_ROUTINES */
#define EPIC_REG_READ(offset)	\
...
#ifdef  _WRS_CONFIG_SMP
#define VXB_SYS_EPIC_ID_GET(physCpuIndex)               \
    _func_vxbSysEpicIdGet == NULL ? physCpuIndex :      \
        _func_vxbSysEpicIdGet (physCpuIndex)
#else
