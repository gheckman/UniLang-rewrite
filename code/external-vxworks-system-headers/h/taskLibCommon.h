...
--------------------
02v,21feb13,cww  Added definition of errno S_taskLib_STOPPED_DELETE_SAFE_TASK
                 (part of fix for WIND00402989).
02u,15jun12,jpb  Defect# WIND00354653.  Backed out removal of VX_PRIVATE_ENV
                 from VX_USR_TASK_OPTIONS_BASE.
02t,15feb12,jpb  #Defect WIND00308068.  Removed VX_PRIVATE_ENV from user
                 options.
02s,09nov10,zl   added TASK_ID_NULL definition
02r,16jun10,pcs  Change the data types of ticks and td_delay from an int to
                 _Vx_ticks_t
02q,27may10,pad  Moved extern C statement after include statements.
02p,07jan10,jpb  Removed user-side definitions which are now the same as kernel
02o,17dec09,jpb  Updated sharing of TASK_DESC structure.
02n,16mar09,zl   fixed taskIdDefault() to return TASK_ID.
02m,02mar09,pcs  Updated to limit the taskLibXXX API signature changes to the
                 kernel, to avoid usr side build errors.
02l,27feb09,pcs  revert back the signature change of taskStackSizeGet.
02k,03feb09,pcs  Updated to add support for the LP64 data model.
02k,20jan09,jb   Adding x86_64 Support
02j,28aug08,nld  added taskIsDelayed()
02i,13jul07,rps  added errno S_taskLib_ILLEGAL_STACK_INFO
02h,14may07,jmg  Moved taskCpuAffinityGet() and taskCpuAffinitySet() to here.
02g,08feb07,lei  added td_affinity to TASK_DESC
02f,18jan07,dlk  Added taskStackSizeGet() prototype.
02e,11aug06,lei  added td_cpuIndex to TASK_DESC for SMP; removed
                 _WRS_VX_SMP
02d,10jul06,jpb  Updated for POSIX namespace conformance.
		 Changed C preprocessor macro CPU_FAMILY to _VX_CPU_FAMILY.
02c,06jul06,jmg  Added extern declaration for taskPriNormalget() as part of
                 Defect #WIND00045158
02b,07feb06,kk   renamed taskOpenExt to taskOpenWithGuard
02a,13dec05,jln  added taskOpenExt
01z,18may05,rec  add hook for coprocColdfire.h
01y,01apr05,kk   remove old coprocessor options, now all archs PAL'ifies
                 coprocs
01x,28sep04,aeg  added td_excStackCurrent to TASK_DESC.
01w,27sep04,tam  updated VX_NO_STACK_PROTECT documentation
01v,22sep04,md   added taskSafe()/taskUnsafe() prototype.
...
#define TASK_DESC_FIELDS(tidType, rtpIdType)						\
    ...
    size_t              td_excStackMargin;/* exception stack margin */			\
