...
--------------------
02q,08dec14,pcs  Added VM_DEFAULT_CACHE_COHERENCY_ATTR to DEFAULT_VM_MAP_ATTR
                 (VXW6-83894).
02p,05nov10,zl   removed VM_PHYS_TRANSLATE() for 64-bit VxWorks.
02o,25oct10,zl   added TASK_MMU_CURRENT_SET()
02n,01jul10,zl   added page optimization field to VM_LIB_INFO
02m,29apr10,pad  Moved extern C statement after include statements.
02l,10sep09,jb   Fix Optimized size prototypes
02k,24aug09,zl   added page optimization info routines, vmPgMap() options.
02j,01jun09,zl   added MMU_ARCH_ATTR.
02i,21apr09,zl   LP64 updates. Replaced msize_t with size_t
02i,21apr09,gls  added MMU_ATTR_NOT_GLOBAL
02h,26sep08,pcs  Fixed types inconsistencies with VM_CONTEXT_BUFFER_WRITE
                 macro arguments. Defect:WIND00084242
02g,27aug08,jpb  Renamed VSB header file
02f,24jun08,jpb  Added include path for kernel configurations options set in
                 vsb.  Renamed _WRS_VX_SMP to _WRS_CONFIG_SMP.
02e,08jun07,kk   added vxSmpP.h
02d,02may07,zl   introduced strong-typed function pointers, removed 
                 vmTransTblGet().
02c,11may07,pcs  Add macro VM_DEFAULT_CACHE_COHERENCY_ATTR
02b,06mar07,pcs  update vmPgMap to add 2 extra params.
02a,13feb07,pes  Add support for _WRS_MMU_HAS_MAP_STATE_API macro.
01z,05jul06,zl   removed mmuPageUnProtect from MMU_LIB_FUNCS.
01y,11aug05,zl   moved common VM macros from the source files
01x,29jun05,zl   removed adrSpaceId parameter from VM_CONTEXT_SWITCH();
...
#if defined (_WRS_MMU_HAS_64BIT_ARCH_ATTR)	&& \
   !defined (_WRS_MMU_HAS_SPL_TRANSLATE_API)
#warning "for 64-bit MMU attributes the new SPL translation API is required"
#endif
...
#ifdef _WRS_ARCH_HAS_TASK_MMU_CURRENT_SET
extern void taskMmuCurrentSet (MMU_TRANS_TBL_ID pTransTbl);
#define TASK_MMU_CURRENT_SET(pTbl)	taskMmuCurrentSet(pTbl)
#else
#define TASK_MMU_CURRENT_SET(pTbl)
#endif

/* 
 * SPL translation is handled as specia case, depending on which API 
 * is supported by the architecture.
 */

#ifdef _WRS_MMU_HAS_SPL_TRANSLATE_API
...
#define MMU_SPL_TRANSLATE(attr, mask, pArchAttr, pArchMask)		\
	((mmuLibFuncs.mmuAttrTranslate == NULL) ? OK :			\
	  (*(mmuLibFuncs.mmuAttrTranslate))				\
			(attr, pArchAttr, mask, pArchMask, TRUE))
#define MMU_SPL_ARCH_TRANSLATE(depAttr, pToIndep)			\
	((mmuLibFuncs.mmuAttrTranslate == NULL) ? OK :			\
	  (*(mmuLibFuncs.mmuAttrTranslate))				\
			(depAttr, pToIndep, 0, NULL, FALSE))
...
typedef struct vmLibInfo
    ...
    BOOL mmuSoftCfgEnabled;		/* TRUE: software MMU simulation */
...
typedef struct
	...
	size_t		numPages,
	BOOL		optimize
#ifdef _WRS_MMU_HAS_MAP_STATE_API
	,MMU_ARCH_ATTR	attr
#endif
	);
...
#else
    STATUS (*mmuAttrTranslate)
	(
	UINT		fromState,
	UINT *		pToState,
	UINT		fromStateMask,
	UINT *		pToStateMask,
	BOOL		independentToDependent
	);
#endif
...
#ifdef _WRS_MMU_HAS_OPTIMIZATION_INFO_API
...
#endif /* _WRS_MMU_HAS_OPTIMIZATION_INFO_API */
    } MMU_LIB_FUNCS;
