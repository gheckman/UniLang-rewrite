...
/*
 * Copyright (c) 2001-2005, 2008-2010 Wind River Systems, Inc.
 *
...
--------------------
02f,08sep10,rlp  Fixed previous merge.
02e,10aug10,v_r  Architecture-specific symbol hook now able to modify symbol
                 characteristics.
02d,27jan10,v_r  Added architecture-specific fully-qualified hook function
...
02b,15apr10,v_r  Added ability to configure the parser in ELF32-only mode.
02a,23feb10,v_r  Revert structure fields reorganization until we can update
                 host side as well.
01z,22feb10,v_r  LP64 TODOS cleanup + structure reorganization.
		 Moved SYMTBL_REFS to loadRtpLib.
01y,29jan10,v_r  Added loadElfSymbolEntryRead() and
		 loadElfSectionHeaderReadAndCheck() declarations.
01x,22jun09,v_r  Removed ELF_CLASS macro.
01w,16jun09,v_r  LP64 adaptation: went on w/ relocation abstraction usage and
		 updated SEGMENT_INFO_DESC to size_t.
01v,09jun09,v_r  LP64 adaptation: added relocation abstraction, allowing to
		 handle REL/RELA 32/64 bits relocs in a unified manner.
01u,04jun09,v_r  LP64 adaptation: added ELF abstraction, allowing to handle
		 ELF64 and ELF32 files in an unified manner.
01t,06may09,v_r  LP64 adaptation (phase #1) : updated code following adaptation
                 guidelines.
		 Removed unused RA_MSK, GPR13_MSK and GPR2_MSK macros and
		 SYMADDR_REFS type.
01s,29apr08,v_r  Now provide ELF header to elfArchVerify() routine (req
		 CQ:WIND00090099).
