...
--------------------
--------------------
01r,18oct13,jxu  cleanup of warnings
01q,04sep13,jxu  fixed incorrect block version issue.
01p,02sep13,jxu  added average age of all data blocks to meta header. 
01o,23aug13,jxu  supported incremental log.
01n,10jul13,jxu  supported clean unmount, and removed group wl info from
                 map page and added grp_wl_sort, and supported per-device
                 user configurations.
01m,26jun13,jxu  fixed incorrect bits of blkVer in BLOCK_INFO. 
01l,25jun13,jxu  optimized the alignment for the packed structures. 
01k,08jun13,jxu  added page status to oob and few flags 
01j,05jan13,jxu  added debugging tool for tracking lpn mapping and ppn status 
01i,07nov12,jxu  added memory statistics for ftl 
01h,06nov12,jxu  added support of ftl discard feature. 
01g,08nov12,jxu  added more info to ftl identity and added wl info to meta log.
01f,22oct12,jxu  added pFpioReqBmp to support doing mapping for multiple
                 non-contiguous page read.
01e,09jul12,jxu  adapted for integration.
01d,18jun12,m_y  added wl related defines.
01c,17may12,x_z  added bm related defines.
01b,15may12,jxu  revised based on internal review.
...
#include <private/ftl/ftlUtilP.h>
...
int fpioListMove (DL_LIST * pDstList, DL_LIST * pSrcList,
            DL_NODE * pNode);
_ftl_inline int fpioListDel (DL_LIST * pList, DL_NODE * pNode);
_ftl_inline int fpioListInit (DL_LIST * pList);
_ftl_inline FTL_PIO * fpioListNext (DL_LIST * pList, DL_NODE * pNode);
_ftl_inline FTL_PIO * fpioListFirst (DL_LIST * pList);
...
_ftl_inline int ftlAgeThreshUpdate (FTL_DESC * pFtl); 
_ftl_inline void ftlAvgAgeUpdate (FTL_DESC * pFtl); 
