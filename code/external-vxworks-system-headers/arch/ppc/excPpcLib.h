...
/*
 * Copyright (c) 1984-2008, 2010-2014 Wind River Systems, Inc.
 *
...
--------------------
02r,04dec14,h_k  removed a wrong comment about WIND_TCB offset.
02q,01oct14,h_k  changed the offset comments in EXC_INFO to relative.
                 added offset comments to pad5 and total size in EXC_INFO.
02p,09apr13,d_l  fix build warnings. (WIND00358283)
02s,26mar13,h_k  added missing fpcsr for PPC465.
02r,21feb13,h_k  added _EXC_ALTIVEC_UNAVAILABLE and _EXC_ALTIVEC_ASSIST for
                 e6500. (WIND00404757)
02q,20nov12,pcs  Fix compiler warning corresponding to macro _OP.
02p,05sep12,h_k  moved _PPC_IMISS_SAVE_AREA and _PPC_DMISS_SAVE_AREA
                 from mmuE500ALib.s.
02o,06apr12,cww  add esr for PPC4xx
02q,30aug12,jpb  Renamed _WRS_CONFIG_MILS_VDK_1_1 to _WRS_CONFIG_MILS_VBI.
02p,28nov11,h_k  updated comments for offset in EXC_INFO.
02o,04nov11,kab  Fix multiple def of _OP compiler warnings
02n,15jun11,kab  Add MILS VDK support
02m,03may11,syt  Add support for PPC465.
02l,08oct10,x_z  Rework mcsr field of EXC_INFO for PPC476.
02k,20aug10,x_z  Add support for PPC476.
02j,22mar10,wap  Add alternate direct interrupt vector for e500mc guest mode
02i,03sep08,pmr  e500mc hard floating point
02h,05jan07,pch  Add _EXC_OFF_SYSTIMER (abstraction of _DECR and _PIT);
		 renumber history from 10dec03.
02g,01jun06,pch  consolidate vector offset definitions in excPpcLib.h
02f,15jul05,mil  General clean up.
02e,22apr04,to   PPC970 cleanup
02d,18mar04,rec  merge PPC970 changes
02c,26aug04,pch  SPR 78780: add notes concerning vector locations.
02b,09jun04,pch  add support for PPC440 with hard FP
02a,03may04,dtr  Add exception message exchandler install struct.
01z,24mar04,dtr  Adding in new struct to support generic TLB and parity
		 exception handlers.
01y,25feb04,pch  SPR 92351: define _EXC_OFF_END for all PPC
01x,07nov02,mil  Added support for PPC85XX.
01w,10dec03,jtp  Code inspection update: 405 TLB Miss handler relocation
01v,26nov03,pes  Correct _WRS_IS_SUPV_EXC macro.
01u,18nov03,pes  Add _WRS_IS_SUPV_EXC macro.
01t,10jun03,pch  move 440x5 MC handler & save area to not clobber bootline
01s,08jan03,pch  Add support for 440x5 core (PPC440GX)
...
#define _EXC_INFO_MSR	0x004	/* machine status register valid */
	 ...
	 (CPU == PPC440) || (CPU == PPC85XX) || (CPU == PPC476) || (CPU == PPC465))
...
typedef struct
    ...
    UINT32      cr;		/* 0x14: condition register */
    UINT32	xer;		/* 0x18: fixed point exception reg */
...
# endif /* PPC_440x5 */
    ...
    UINT32	fpcsr;		/* floating point status reg */
    UINT32	mcsr;
#elif (CPU == PPC85XX)        	/* excluding MCSR */
# ifdef _WRS_SPE_SUPPORT
    UINT32	spefscr;	/* SPE float-pt status reg */
# else
    UINT32	fpcsr;		/* floating point status reg */
# endif 
    UINT32	mcsr;		/* MCSR syndrome reg */
#else	/* CPU == PPC4xx */
...
#if ((CPU == PPC403) || (CPU == PPC405) || (CPU == PPC405F) || \
     (CPU == PPC440) || (CPU == PPC476) || (CPU == PPC465) || (CPU == PPC85XX))
...
#if (CPU == PPC85XX) && defined(_WRS_CONFIG_SMP)
#ifdef	PPC_e6500
/* use 0x2400-0x27ff for ITLB miss temp storage area for upto 32 cores */

#define _PPC_IMISS_SAVE_AREA	0x2400

/* use 0x2800-0x2bff for DTLB miss temp storage area for upto 32 cores */

#define _PPC_DMISS_SAVE_AREA	0x2800
#else
/* use 0x1d00-0x1dff for ITLB miss temp storage area for upto 8 cores */

#define _PPC_IMISS_SAVE_AREA	0x1d00

/* use 0x1f00-0x1fff for DTLB miss temp storage area for upto 8 cores */

#define _PPC_DMISS_SAVE_AREA	0x1f00
#endif	/* PPC_e6500 */
#endif	/* CPU == PPC85XX && _WRS_CONFIG_SMP */
