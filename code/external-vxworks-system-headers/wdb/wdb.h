...
/*
 * Copyright (c) 1995,1999-2000,2003-2011,2013 Wind River Systems, Inc.
 *
...
--------------------
04b,02apr13,rlp  Added new WDB error + context delete force option (part of fix
		 for defect WIND00402989).
04a,08apr11,rlp  Added WDB ioctl numbers.
03z,16mar11,elp  added VFP registers.
03y,29jul10,pad  Moved extern C statement after include statements.
03x,10mar10,rlp  Modified memSize field of WDB_RT_INFO for LP64.
03x,17mar10,rlp  Fixed WDB_CALL_RET_INFO structure to handle returnValLong
                 value correclty on 32-bit host.
03w,05feb10,rlp  Renamed loCtx field of WDB_MEM_CTX_T to ctx for backward
                 compatibility.
03v,10dec09,rlp  Modified WDB_CALL_RET_INFO structure for LP64 adaptation.
03u,08oct09,rlp  Modified WDB version number
		 Modified WDB_MEM_CTX_T structure for LP64 adaptation.
03u,11jun09,rlp  Defined numBytes of WDB_MEM_REGION as an INT32 (WIND00169866).
03t,15sep08,jmp  changed value for WDB_TSFS_O_TEXT on runtime side to resolve
		 conflict with O_NONBLOCK flag which had the same value
		 (CQ:WIND00119715).
...
/* WDB I/O contron function numbers */

#define	WDB_IOCTL_DRV_DISABLE		1	/* Disable WDB agent driver */
#define	WDB_IOCTL_DRV_ENABLE		2	/* Enable WDB agent driver */
#define	WDB_IOCTL_DRV_SHUTDOWN		3	/* Shutdown WDB agent driver */
...
#define	WDB_ERR_IOCTL_NOT_SUPP		0x516	/* IOCTL Op. not supported */
#define WDB_ERR_DELETE_SAFE_CONTEXT	0x517	/* Context is in delete */
						/* safe mode */
...
/*
 * XXX The following macro should be changed at least 2 releases after VxWorks
 * 6.7:
 * Until VxWorks 6.6, we were documenting to use this macro to open a TSFS
 * file in text translation mode, but the macro value (0x4000) was in conflict
 * with O_NONBLOCK flag (See CQ:WIND00119715). Since VxWorks 6.7, we document
 * to use O_TEXT flag instead, but in order to preserve source compatibility
 * for 2 releases, we've kept this macro but changed it to O_TEXT.
 * The tgtsvr/agent protocol remain unchanged: they still use the old macro
 * value (0x4000) but using WDB_TSFS_O_TEXT2 macro on runtime side.
 *
 * XXX Should be (for host and target side):
 *
 * #define WDB_TSFS_O_TEXT	0x4000	/@ Text translation mode @/
 */
#define	WDB_TSFS_O_TEXT		0x200000 /* Text translation mode: */
					 /* This value be equal to O_TEXT */
					 /* flag defined in fcntlcom.h */

#define WDB_TSFS_O_TEXT2	0x4000	 /* Runtime Text translation mode */
#else /* _WRS_KERNEL */
#define WDB_TSFS_O_TEXT		0x4000	/* Host side Text translation mode */
					/* Must be equal to the runtime */
					/* WDB_TSFS_O_TEXT2 macro */
#endif	/* !_WRS_KERNEL */
...
/* context deletion options */

#define	WDB_CTX_FORCE_DELETE	0x2		/* force deletion of context */
...
#define WDB_REG_SET_VFP		9	/* VFP registers */
...
/*
 * Even if the following structure does not make sense (LSB and MSB parts of
 * the memory context id are not consecutive), we must not change this for
 * backward compatibility issue.
 */

typedef struct
    {
    TGT_INT_T		ctx;		/* LSB of the memory context id */
    WDB_POINTER_SIZE	pointerSize;
    TGT_INT_T		hiCtx;		/* MSB of the memory context id */
    } WDB_MEM_CTX_T;
...
typedef struct wdb_core			/* common header WDB structure */
    {
    UINT32		numInts;	/* WDB_CORE length: 4 */
...
typedef struct wdb_mem_region		/* a region of target memory */
    ...
    INT32		numBytes;	/* memory region size */
...
typedef struct wdb_rt_info		/* info on the run-time system */
    ...
    TGT_ADDR_T		memBase;	/* target main memory base address */
    					/* valid for VxWorks 32-bit only */
    TGT_LONG_T		memSize;	/* target main memory size */
...
typedef struct wdb_call_ret_info
    {
    TGT_LONG_T		callId;		/* returned from WDB_FUNC_CALL */
