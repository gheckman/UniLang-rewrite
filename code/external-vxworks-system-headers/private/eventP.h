...
/*
 * Copyright (c) 1994-2011 Wind River Systems, Inc.
 *
...
--------------------
01l,05may15,wzc  fix the atomic operation.(VXW6-84315)
01k,29aug11,wzc  Update to support dynamic power management (WIND00295115)
04j,27jul11,wzc  PPM for SV (WIND00214625)
04i,31jan11,jpb  Defect WIND00253474. Fixed build warning.
04h,22nov10,wzc  Fix the LITTLE_ENDIAN issue of EVT_STORE(WIND00242104)
04g,01oct10,vhe  Fix shifts in LP64 for unaligned support
04f,24sep10,jpb  Added global variable evtAction.  Moved from funcBindP.h
04e,21jun10,vhe  Add cast to PC for different arch
04d,31may10,wzc  Moved EVT_CTX_BUF_T to EVT_WVEVENT (WIND00214658).
04b,26may10,pad  Moved extern C statement after include statements.
04c,26may10,tcr  add EVT_CTX_NODISP back
04b,10feb10,tcr  Update for LP64 System Viewer
04a,26may09,tcr  create optimized CTX_DISP event
03z,29sep08,tcr  change intrumentation test if OBJECT_CLASS_LIST excluded
03y,16sep08,tcr  Fix WIND00130140: Change while(0) to while((0))
03x,16jul08,to   rename _WRS_EXPECT_UNLIKELY to _WRS_UNLIKELY
04d,25jul08,tcr  update macro
04c,04jul08,tcr  update structures
04b,13jun08,tcr  change evtAction modifiers to atomic
04a,09may08,tcr  fix tasknames preservation
03z,02may08,tcr  move event definitions out
03y,01may08,aim  call eventPointSwHandle in EVT_CTX_TASKINFO
03x,17mar08,tcr  change macros to function calls
03w,12may08,to   use _WRS_EXPECT_UNLIKELY for logging path
03v,21sep07,tcr  correct EVT_CTX_RTPINFO macro (WIND00105753)
03u,12jun07,tcr  more SMP updates
03t,19dec06,tcr  add revision id for SMP
03s,15feb07,jmg  Added level 2 event EVENT_WINDROTATE for windRotate
03r,09jan07,jmg  Added EVENT_TASKROTATE (Req. WIND00071010)
03q,23jan06,tcr  Change log revision ID to track syscall changes
...
#define TRG_ACTION_SET                  \
        {                               \
        vxAtomic32Or ((atomic32_t*)&evtAction, 0x0001);            \
        }

#define TRG_ACTION_UNSET                \
        {                               \
        vxAtomic32And ((atomic32_t*)&evtAction, ~(0x0001));         \
        }

#define WV_ACTION_SET                   \
        {                               \
        vxAtomic32Or ((atomic32_t*)&evtAction, 0x0100);\
        }

#define WV_ACTION_UNSET                 \
        {                               \
        vxAtomic32And ((atomic32_t*)&evtAction, ~(0x0100));         \
        }
...
/* FIXME This doesn't handle the payload or class/event id */
...
#define WV_REV_ID_CURRENT_SMP           (WV_REV_ID_VXWORKS_6_9_2 | WV_REV_ID_SMP)
#define WV_REV_ID_CURRENT               WV_REV_ID_VXWORKS_6_9_2
