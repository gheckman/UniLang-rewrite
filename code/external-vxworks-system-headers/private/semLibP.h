...
/*
 * Copyright (c) 1984-2010, 2013-2014 Wind River Systems, Inc.
 *
...
--------------------
03c,22oct14,pcs  made semTerminate() public (VXW6-34630).
03b,07nov13,cww  Merge in semi-scalable inline RW semaphores.
03a,09nov10,gls  Added VX_OBJ_ALIGN_SIZE aligment to SEMAPHORE typedef
02z,13oct10,jpb  Fixed wrong offset listed in comments of SEMAPHORE struct
02y,09sep10,jpb  Removed unused function pointer semSmInfoRtn.
02x,31aug10,cww  Use dynamically generated offsets in offset macros
02w,27aug10,s_s  SEM_TYPE_MASK value is changed to 0x7 as the maximum number
                 of semaphores supported is 8
02v,26jul10,s_s  Fix for the defect WIND00207200
02u,16jun10,pcs  change the data type of timeout from an int to _Vx_ticks_t
02t,07may10,pch  reduced semaphore table size when using small footprint
02s,06may10,cww  Updated SEM_OWNER for LP64
02r,03may10,cww  Fix size mismatch in SMP version of SEM_OWNER
02q,29apr10,pad  Moved extern C statement after include statements.
02p,23feb10,gls  added SEMC_PENDED_TASK
02o,22dec09,jpb  Introduced new kernel-side structure SEM_INFO_USR.
02n,22dec09,gls  added SEM_KERNEL_INTERRUPTIBLE (WIND00193764)
02m,21dec09,pcs  Fixed an issue with the macro enclosing the offsets into the
		 semaphore (used by optimized semaphore libraries.)
02l,09mar09,pcs  Updated to add support for the LP64 data model.
02k,11feb09,zl   replaced qLib.h with qLibP.h
02j,31jul09,jpb  Fixed build warnings.
02i,01may09,gls  added semMTakeByProxyNoLock() for SMP
02h,17jun08,gls  added inline give/take routines
02g,13dec07,gls  updated MAX_SEM_TYPE and SEM_TYPE_MASK
02f,04jun07,gls  added non-locking give and take routines
		 added various semaphore function pointers
02e,19jan07,gls  updated reader/writer semaphore support
02d,23oct06,jpb  Removed deprecated functions and message for semQInit
		 (Defect# 35589).
02c,13oct05,yvp  Changes text of deprecated messages - use semXInitialize.
02b,26aug05,gls  fixed pSemExt index comment (SPR #111547)
02a,04aug05,gls  added read/write semaphore support
01z,04aug05,yvp  Added prototype for create/delete library init.
01y,07jun05,yvp  Updated copyright. #include now with angle-brackets.
01x,12apr05,kk   start deprecation of semXCoreInit() and semQInit().
01w,28sep04,fr   removed vSemCreate prototype (SPR 101349)
01v,10sep04,fr   declared semCreateTbl (SPR 101247)
01u,22mar04,dcc  added vSemCreate prototype.
01t,09mar04,dcc  updated SEMAPHORE offsets.
01s,08mar04,aeg  removed Diab _WRS_DEPRECATED C++ workaround.
01r,02mar04,aeg  deprecated semOLib APIs.
01q,31oct03,md   Added semMTakeByProxy
...
/*
 * The SMP implementations for the take and give of a binary sempahore
 * use the following definitions for the "empty" and "full" state of a
 * sempahore.  This contrasts with the UP implementation which uses the value
 ...
 * task has attempted a take operation while the sempahore was "empty".
 */
