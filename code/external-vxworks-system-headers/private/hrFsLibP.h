...
-------------------- 
03e,02jul13,myt  WIND00424679 - restore all operation to atomic semantic
                 in default mode
03f,05mar13,myt  WIND00401243 - initialize semaphore when mounting hrfs instead of
                 in every transaction
03d,13sep12,myt  WIND00368976 - add two varible to record freeInode and 
                 freeDataBlk in this trans avoid to be changed by next 
                 trans in hrfsHighSpeedWriteMode.
03c,12myt11,myt  add a writing superblock num macro definition 
03b,13jul11,jxu  add dirRenameSlot for rename operation (WIND00287328).
03a,06apr11,jxu  WIND00252514 - HRFS is soon corrupt once full 
                 reserved constant number of blocks for file deletion or 
                 truncation when disk is full. 
02z,09apr10,lxj  adapted for LP64 support 
02y,28oct09,hui  manual merge to 64 bit
02x,01oct09,gls  removed use of POSIX APIs
02w,01jul09,hui  WIND142041 - HRFS performance improvement
02v,02jun09,xia  Modified for hrfsAscTime()
02u,19aug08,hui  WIND000129267 - HRFS poor performance when performing more than 
                 one writes.
02t,22jan07,act  add discard-interface info
02s,03nov06,jlk  WIND00083334 - File deletion strategy change.
02r,26may06,jlk  added stuff for commit policy changes
02q,04jan06,act  add POSIX file locking
02p,16nov05,md   added time function protos and in-core structures
02o,01nov05,pcm  made direct, indirect, indirect2 and indirect3 into an array;
...
#define  HRFS_VOP_NONDELETE     0                
#define  HRFS_VOP_UNLINK        1
#define  HRFS_VOP_INACTIVATE    2
#define  HRFS_VOP_RMDIR         4
#define  HRFS_VOP_TRUNC         8
...
int hrfsFindTriePath(struct vnode *, UINT32, HRFS_INODE_PTR,
...
extern int  hrfsVopPathConf	(struct vnode * vp, int name, long * retval);
