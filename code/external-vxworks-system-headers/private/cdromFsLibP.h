...
/* 
 * Copyright (c) 2002, 2004-2005, 2010, 2013 Wind River Systems, Inc. 
 *
...
--------------------
01l,27mar13,xwu  fix WIND00405848 In cdromFsLib, the way clones of T_CDROM_FILE
                 are used causes race conditions and reentrancy issues.
01k,29apr10,pad  Moved extern C statement after include statements.
01j,11aug05,aeg  moved cdromFsDevDelete() prototype to h/cdromFsLib.h
01i,05aug05,aeg  added cdromFsDevDelete()+ function prototypes.
01h,31may05,aeg  added cdromFsMonitorDevCreate() and cdromFsDiskProbe().
01g,31mar05,aeg  changes to convert to XBD device model.
01e,14sep04,smz  Added mFdListSem mutex for fdList operations
01d,16aug04,smz  Added mDevSem to T_CDROM_FILE to support mutex protection in
                 close operation.
...
#include <private/semLibP.h>	/* SEMPAHORE, semBInit */
#include <time.h>
...
typedef struct secBuf	/* buffer for reading sectors */
    {
    u_long	startSecNum;	  /* logical sector number */
    u_long	numSects;	  /* Number of logical sectors contained */
    u_long	maxSects;	  /* max # of logical sectors it can contain */
    int		magic;		  /* SEC_BUF_MAG after sectData allocation */
    struct bio  bio;		  /* XBD bio structure */
    SEM_ID      bioDoneSyncSem;   /* binary sempahore used for bio_done */
    } SEC_BUF;
...
typedef struct cdromVolDescr	/* CDROM_VOL_DESC */
    ...
    u_int      sectSize;	/* logical sector size.  Divide by */
				/* LSToPhSSizeMult to get physical sect size */
    ...
    SEM_ID     mFdListSem;      /* FdList mutual-exclusion semaphore */
    ...
    u_char     LSToPhSSizeMult; /* LSSize/PhSSize (needed when XBD physical */
    ...
    sector_t   numSectors;	/* number of physical sectors on media */
    } CDROM_VOL_DESC;
...
extern  STATUS cdromFsFsmInit 		(void);
extern  void   cdromFsBioDone		(struct bio * pBio);

extern  CDROM_VOL_DESC_ID _cdromFsDevCreate (char * devName,
					     device_t device,
					     BOOL fsmCreate);
