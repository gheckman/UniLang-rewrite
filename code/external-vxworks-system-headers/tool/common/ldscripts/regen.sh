...
# --------------------
# 01h,06jan12,my_  add support for ARMARCH4
# 01g,10mar10,gvm  updates for icc
# 01f,29jul05,tpw  Document order dependency for MIPS romResident fix.
# 01e,06may05,rlg  changes to add mips specific scripts
...
case "$TOOL" in
    gnu|icc)	tool_script_list="AOUT,RAM AOUT,ROM OUT" ;;
    *)		tool_script_list=""			;;
esac

# MIPS,ROM must appear in that order to be caught by the kseg0data_data check.
#
common_script_list="RAM ROM DOTBOOT,RAM DOTBOOT,ROM ARMARCH4,ROM MIPS,RAM MIPS,ROM"
