/* ppc403Sio.h - header file for PPC403GA SPU driver */
...
01a,13feb96,tam  written (from i8250Sio.h)
*/

#ifndef	__INCppc403Sioh
#define	__INCppc403Sioh
...
typedef struct PPC403_CHAN
    ...
    UCHAR   (*inByte) (int);         	/* routine to read a byte from reg. */
    void    (*outByte)(int,char);    	/* routine to write a byte to reg. */
    
    int     baudRate;			/* baud rate */
    UINT32  clkFreq;			/* baud rate generator clock */
    uint_t      mode;			/* SIO_MODE_[INT | POLL] */

    volatile char *spctl;       	/* spu control register */
    volatile char *sphs;                /* spu handshake status register */
    volatile char *spls;                /* spu line status register */
    volatile char *sprb;                /* spu receive buffer */
    volatile char *sprc;                /* spu receiver command register */
    volatile char *sptb;                /* spu transfer buffer */
    volatile char *sptc;                /* spu transfer command register */
    } PPC403_CHAN;

#define _TY_RX_VEC      4
#define _TY_RX_INT_LVL  4
#define _TY_TX_VEC      5
#define _TY_TX_INT_LVL  5
...
extern void ppc403DevInit (PPC403_CHAN *pDev);
extern void ppc403IntRd (PPC403_CHAN  *pDev);
extern void ppc403IntWr (PPC403_CHAN  *pDev);

#else

extern void ppc403DevInit ();
extern void ppc403IntRd ();
extern void ppc403IntWr ();
